...
#pragma GCC diagnostic pop
  ...
  std::thread session_monitor_thread([&]() {
    ...
    WNDCLASSA wnd_class {};
    wnd_class.lpszClassName = "RuntimeBrokerSessionMonitorClass";
    ...
    auto wnd = CreateWindowExA(
      ...
      "RuntimeBroker Session Monitor Window",
      0,
      CW_USEDEFAULT,
      CW_USEDEFAULT,
      CW_USEDEFAULT,
  ...
  on_signal(SIGINT, [&force_shutdown, &display_device_deinit_guard, shutdown_event]() {
    ...
    auto task = []() {
      BOOST_LOG(fatal) << "10 seconds passed, yet RuntimeBroker's still running: Forcing shutdown"sv;
      logging::log_flush();
      lifetime::debug_trap();
    };
  ...
  if (config::sunshine.port == 47989 && is_gamestream_enabled()) {
    BOOST_LOG(fatal) << "GameStream is still enabled in GeForce Experience! This *will* cause streaming problems with RuntimeBroker!"sv;
    BOOST_LOG(fatal) << "Disable GameStream on the SHIELD tab in GeForce Experience or change the Port setting on the Advanced tab in the RuntimeBroker Web UI."sv;
  }
#endif
