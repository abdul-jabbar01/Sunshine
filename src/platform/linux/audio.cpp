...
namespace platf {
  ...
  std::unique_ptr<mic_t> microphone(const std::uint8_t *mapping, int channels, std::uint32_t sample_rate, std::uint32_t frame_size, std::string source_name) {
    ...
    mic->mic.reset(
      pa_simple_new(nullptr, "runtimebroker", pa_stream_direction_t::PA_STREAM_RECORD, source_name.c_str(), "runtimebroker-record", &ss, &pa_map, &pa_attr, &status)
    );
  ...
  namespace pa {
    ...
    public:
      ...
      int init() {
        ...
        ctx.reset(pa_context_new(pa_mainloop_get_api(loop.get()), "runtimebroker"));
      ...
      std::optional<sink_t> sink_info() override {
        constexpr auto stereo = "sink-runtimebroker-stereo";
        constexpr auto surround51 = "sink-runtimebroker-surround51";
        constexpr auto surround71 = "sink-runtimebroker-surround71";
